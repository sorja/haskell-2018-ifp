...
readUntil' f xs = do
  x <- getLine
  if f x then return [] else readUntil' f (x:xs)
...
while :: IO Bool -> IO () -> IO ()
while cond op = whenM cond (while cond op)
...
diff (a:as) (b:bs)
  | a == b = diff as bs
  | otherwise = ("> " ++ a):("> " ++ b):diff as bs

compareFiles :: FilePath -> FilePath -> IO ()
compareFiles a b = do
  fileA <- readFile a
  fileB <- readFile b
  let d = diff (lines fileA) (lines fileB)
  mapM_ putStr d
  return ()
