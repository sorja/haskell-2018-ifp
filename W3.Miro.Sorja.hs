...
-- mapMaybe f xs = map (filter (/= Nothing) (map f xs))
...
treeSize :: Tree a -> Int
treeSize Leaf = 0
-- treeSize (Node _ a _) = 1 + treeSize a
-- treeSize (Node _ a _) = 1 + treeSize a
-- treeSize (Node _ _ b) = 1 + treeSize b
...
insertL x (Node v l r) = Node v (insertL x l) r
...
measure Leaf = Leaf
measure (Node v l r) = Node (treeSize (Node v l r)) (measure l) (measure r)
