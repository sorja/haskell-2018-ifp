...
tribonacci n = tribonacci' 2 1 1 n

tribonacci' :: Integer -> Integer -> Integer -> Integer -> Integer
tribonacci' a _ _ 3 = a
tribonacci' _ b _ 2 = b
tribonacci' _ _ c 1 = c
tribonacci' a b c n = tribonacci' (a+b+c) a b (n-1)
...
funnyCompare a b | odd a && odd b = compare a b
...
funnyMin a b = funnyMin' a b (funnyCompare a b)

-- Hmm, wish material had example of case-of (what is google)
funnyMin' :: Int -> Int -> Ordering -> Int
funnyMin' a b LT = a
-- funnyMin' a b GT = b
funnyMin' a b _ = b
...
-- pyramid n = "pyramid " ++ show n ++ " ==> " ++ pyramid' 0 n ""
pyramid 0 = "0"
pyramid n = pyramid' 0 n ""

-- Odd 'up', even 'down' # thought 1
-- i = index, curr pos
-- n = mid point
-- s = string
pyramid' :: Integer -> Integer -> String -> String
pyramid' i n s | i <= n       = pyramid' (i+1) n (s ++ show i ++ ",")
               | i < (2 * n)  = pyramid' (i+1) n (s ++ (show ( n - (i `mod` n )))  ++ ",")
               | otherwise    = s ++ show 0
...
smallestDivisor' n k | n `mod` k == 0 = k
